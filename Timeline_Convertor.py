...
class TimelineConverter:
    ...
    def to_kml(self) -> str:
        ...
        # Create folder for visits
        visits_folder = ET.SubElement(document, 'Folder')
        ET.SubElement(visits_folder, 'name').text = 'Places Visited'
        
        for visit in self.visits:
            placemark = ET.SubElement(visits_folder, 'Placemark')
        ...
        activities_folder = ET.SubElement(document, 'Folder')
        ET.SubElement(activities_folder, 'name').text = 'Activities'
        
        for activity in self.activities:
            if len(activity['path']) < 2:
                continue
                
            placemark = ET.SubElement(activities_folder, 'Placemark')
